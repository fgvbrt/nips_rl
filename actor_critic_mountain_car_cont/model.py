...
def build_actor(state_size,  num_act,
    ...
    #l_last = lasagne.layers.DenseLayer(l_hid, num_act*2, W=lasagne.init.Constant(0), nonlinearity=None)
...
def build_critic(state_size, hid_sizes=(50,),
                 nonlinearity=lasagne.nonlinearities.tanh):
    l_input = lasagne.layers.InputLayer((None, state_size))
    l_hid = l_input
    for hid_size in hid_sizes:
    ...
    #l_baseline = lasagne.layers.DenseLayer(l_hid, 1, W=lasagne.init.Constant(0), nonlinearity=None)
...
def build_model(state_size, num_act,
    ...
    # policy loss
    entropy = 0.5*num_act*(1. + T.log(2.*np.pi)) + T.sum(T.log(sigma + 1e-7), axis=1)
    log_prob = -1. * T.sum(T.log(sigma + 1e-7), axis=1) \
    ...
    #updates[state_mean] = (1 - alpha) * state_var + alpha *T.var(states, axis=0)
