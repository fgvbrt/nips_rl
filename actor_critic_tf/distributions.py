...
class DiagGaussianPd(Pd):
    def __init__(self, flat):
        ...
        self.mean = mean
        self.logstd = tf.clip_by_value(logstd, -1e3, -1.)
        self.std = tf.exp(self.logstd)
        #self.std = tf.nn.softplus(logstd) / 10.
        #self.logstd = tf.log(self.std + 1e-7)
    def flatparam(self):
        return self.flat
    def mode(self):
        return self.mean
    def neglogp(self, x):
        return 0.5 * tf.reduce_sum(tf.square((x - self.mean) / self.std), axis=len(x.get_shape()) - 1) \
               + 0.5 * np.log(2.0 * np.pi) * tf.to_float(tf.shape(x)[-1]) \
               + tf.reduce_sum(self.logstd, axis=len(x.get_shape()) - 1)
    def kl(self, other):
        assert isinstance(other, DiagGaussianPd)
        return tf.reduce_sum(other.logstd - self.logstd + (tf.square(self.std) + tf.square(self.mean - other.mean)) / (2.0 * tf.square(other.std)) - 0.5, axis=-1)
    def entropy(self):
        return tf.reduce_sum(self.logstd + .5 * np.log(2.0 * np.pi * np.e), -1)
    def sample(self):
