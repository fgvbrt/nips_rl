...
def run_agent(actor, state_transform, data_queue, weights_queue, process, global_step, best_test_reward,
              testing_period, num_test_episodes, max_steps=10000000):

    env = RunEnv2(state_transform, max_obstacles=3)
    random_process = OrnsteinUhlenbeckProcess(theta=.15, mu=0., sigma=.2, size=env.noutput,
                                              sigma_min=1e-4, n_steps_annealing=1e6)
    ...
    while global_step.value < max_steps:
        ...
        report_str = 'Global step: {}, steps/sec: {:.2f}, episode length {}, reward: {:.2f}, best reward: {:.2f}'. \
            format(global_step.value, 1. * global_step.value / (time() - start), steps, total_reward, best_test_reward.value)
        print report_str
